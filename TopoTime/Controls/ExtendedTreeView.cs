...
namespace TopoTime
{
    ...
    public class ExtendedTreeView : TreeView, IEnumerable, IEnumerable<TreeNode>
    {
        ...
        private const string ResourcePath = "TopoTime.Resources.";
        ...
        private TreeLevelTracker treeLevelTracker;
        #endregion
        ...
        private int CurrentMaxNodeWidth { get; set; }
        public int MaxVisibleLevel
        {
            get { return treeLevelTracker.MaxVisibleLevel; }
        }
...
#pragma warning restore 0067
        ...
        public ExtendedTreeView() : base()
        {
            ...
            treeLevelTracker = new TreeLevelTracker();
        ...
        protected override void OnDrawNode(DrawTreeNodeEventArgs e)
        {
            ...
            try
            {
                ...
                bool isLeaf = !node.IsExpanded || node.Nodes.Count == 0;
                ...
                else
                {
                    ...
                    {
                        bgColor = node.BackColor.IsEmpty ? base.BackColor : node.BackColor;
                        fgColor = node.ForeColor.IsEmpty || node.ForeColor.A == 0 ? base.ForeColor : node.ForeColor;
                    }
                    else
                    {
                        if (base.FullRowSelect)
                ...
                if (isLeaf)
                {
                    extArgs.LineWidth = indent * (MaxVisibleLevel - node.Level + 1) - (height / 2) - 5;
                }
                else
                ...
                if (isLeaf)
                    extArgs.TextBounds = new Rectangle(bounds.Left + indent * (MaxVisibleLevel - node.Level) + 1, bounds.Top, 0, 0);
                else
                ...
                {
                    //DrawSideGroup(g, node, node.Text, node.Bounds);
                }
        ...
        protected override void OnBeforeCollapse(TreeViewCancelEventArgs e)
        {
            ...
            {                   
                ...
                {
                    IEnumerable<TreeNode> descendants = GetExpandedDescendants(e.Node);
                    foreach (TreeNode descendant in descendants)
                    {
                        treeLevelTracker.NodesByLevel[descendant.Level].Remove(descendant);
                    }
                    treeLevelTracker.NodesByLevel[e.Node.Level].Add(e.Node);
        ...
        protected override void OnBeforeExpand(TreeViewCancelEventArgs e)
        {
            ...
            if (!IsLoading)
            {
                if (!e.Node.IsExpanded)
                {
                    ...
                    foreach (TreeNode descendant in descendants)
                    {
                        treeLevelTracker.NodesByLevel[descendant.Level].Add(descendant);
                    }
                    treeLevelTracker.NodesByLevel[e.Node.Level].Remove(e.Node);
                }
            }
        }
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            if (hit.Node != null)
            {
                ...
                {
                    if (hit.Location == TreeViewHitTestLocations.RightOfLabel && !hit.Node.IsExpanded || hit.Node.Nodes.Count == 0)
                    {
                        Point originalLocation = e.Location;
                        originalLocation.X -= MaxVisibleLevel * base.Indent;

                        hit = base.HitTest(originalLocation);
                        if (hit.Location == TreeViewHitTestLocations.Label || hit.Location == TreeViewHitTestLocations.Indent)
                            base.SelectedNode = hit.Node;
                    }
        ...
        public void LoadTree(TreeNode root, IEnumerable<TreeNode> nodeList)
        {
            this.IsLoading = true;
            //treeLevelTracker.MaxVisibleLevelChanged -= RefreshVisibleLeaves;
            CurrentMaxNodeWidth = 0;
            treeLevelTracker.UpdateTreeLevels(nodeList);
            ...
            {
                if (node.Bounds.Width > CurrentMaxNodeWidth)
                    CurrentMaxNodeWidth = node.Bounds.Width;
            }

            this.IsLoading = false;
            //reeLevelTracker.MaxVisibleLevelChanged += RefreshVisibleLeaves;
        }
        ...
        /// <summary>
        /// This yields an enumerable of all descendants, stopping where a node is collapsed.
        /// </summary>
        /// <param name="parent"></param>
        /// <returns></returns>
        private static IEnumerable<TreeNode> GetExpandedDescendants(TreeNode parent)
        {
            foreach (TreeNode descendant in parent.Nodes)
            {
                yield return descendant;
                if (descendant.IsExpanded)
                    foreach (TreeNode nextDescendant in GetExpandedDescendants(descendant))
                        yield return nextDescendant;
            }
        }

        public void DrawSideGroup(Graphics g, TreeNode node, string text, Rectangle newBounds)
        {
            int expandedNodeCount = CountExpandedNodes(node);

            int x1 = MaxVisibleLevel * node.TreeView.Indent + 200;
            int x2 = x1;

            int y1 = newBounds.Y + newBounds.Height / 10;
            int y2 = newBounds.Y + expandedNodeCount * newBounds.Height - (newBounds.Height / 10);

            g.DrawLine(new Pen(Color.Black, 1), x1, y1, x2, y2);

            g.DrawString(text, this.Font,
            new SolidBrush(Color.Black),
            new Rectangle(x1, y2 - (y2 - y1) / 2 - newBounds.Height / 2, newBounds.Width, newBounds.Height));
        }
        ...
        private void RefreshVisibleLeaves()
        {
            int startHeight = this.AutoScrollOffset.Y;
            int endHeight = startHeight + this.Height;
            int nodeTextHeight = this.Nodes[0].Bounds.Height;

            /*
            this.BeginUpdate();
            for (int i = startHeight; i < endHeight; i += nodeTextHeight)
            {
                TreeViewHitTestInfo hit = HitTest(0, i);
                TreeNode hitNode = hit.Node;

                if (hitNode != null)
                {
                    if (hitNode.Nodes.Count == 0 || hitNode.IsExpanded)
                        hitNode.Text = hitNode.Text;
                }
            }
            this.EndUpdate();
            */
        ...
        public bool GetNodeChecked(string name)
        {
            ...
            if (node == null)
                throw new ArgumentOutOfRangeException("name", name, "ExNodeNameOutOfRange");
    ...
    public class TreeLevelTracker
    {
        public Dictionary<int, ObservableUniqueCollection<TreeNode>> NodesByLevel;
        private Dictionary<ObservableUniqueCollection<TreeNode>, int> LevelsByNode;

        public int MaxVisibleLevel { get; set; }

        public TreeLevelTracker()
        {
            NodesByLevel = new Dictionary<int, ObservableUniqueCollection<TreeNode>>();
            LevelsByNode = new Dictionary<ObservableUniqueCollection<TreeNode>, int>();
        }

        public void UpdateTreeLevels(IEnumerable<TreeNode> nodes)
        {
            NodesByLevel.Clear();
            LevelsByNode.Clear();
            ...
            foreach (TreeNode node in nodes)
            {
                ...
                if (!NodesByLevel.TryGetValue(level, out ObservableUniqueCollection<TreeNode> nodeList))
                {
                    nodeList = new ObservableUniqueCollection<TreeNode>();
                    nodeList.CollectionChanged += Source_CollectionChanged;
                    NodesByLevel[level] = nodeList;
                    LevelsByNode[nodeList] = level;
                }
        ...
        {
            ObservableUniqueCollection<TreeNode> nodeLevelList = (ObservableUniqueCollection<TreeNode>)aSender;
            switch (aArgs.Action)
            {
                case NotifyCollectionChangedAction.Add:
                    if (nodeLevelList.Count == 1)
                    {
                        // collection is no longer empty, update list
                        int level = LevelsByNode[nodeLevelList];
                ...
                case NotifyCollectionChangedAction.Remove:
                    if (nodeLevelList.Count == 0)
                    {
                        // collection is now empty, update list
                        int level = LevelsByNode[nodeLevelList];
